...
function zerogravity_scripts_styles() {
	...
	wp_enqueue_style( 'zerogravity-style', get_stylesheet_uri(), false, time() );
...
function zerogravity_excerpt_more($more) {
   global $post;
   return '... <a href="'. get_permalink($post->ID) . '">' . __( 'Читать далее', 'zerogravity' ) . ' &raquo;</a>';
}
...
function table_short($atts) {
	    extract(shortcode_atts(array(
     "img_ref" => '',
     "title" => 'База СТБ'
     ), $atts));
     return '<div style="border: 0.3rem solid #D7414A; float: left; min-width: 300px; background: linear-gradient(to top, #f2f2f2 0%, #ffffff 50%, #f2f2f2 100%); margin: 0 2px 0 2px; max-width: 30%;">
<div style="height: 208px; width: 100%;"><a href="$href"><img class="  wp-image-216  aligncenter" src="'.$img_ref.'" alt="" width="248" height="217" /></a></div>
<div style="min-height: 100px;">
<h2><strong><span style="color: #d7414a; vertical-align: top; margin: 0 0 0 5px;">Асбестоцементная труба </span></strong></h2>
</div>
<div style="background: #D7414A; float: left; font-size: 1.6rem; width: 60%; position: relative; margin: 0 0 20px 0;"><span style="color: #ffffff; float: left; padding: 20px 0 17px 0; margin: 0 0 0 5px;">1шт.</span></div>
<div style="float: right; font-size: 1.5rem; position: relative; width: 40%; background: #D7414A;">
<p style="border-radius: 5px; background: white; min-width: 110px; text-align: center; float: right; width: 35%; margin: 20px 5px 19px 10px;"><span style="color: #d7414a;">720 руб.</span></p>

</div>
</div>';
}
 
add_shortcode( 'table', 'table_short' );
